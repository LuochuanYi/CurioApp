...
import { useBilingualContent, useLocalizedStory } from '../hooks/useBilingualContent';
...
const StoryDetailScreen = ({ navigation, route }) => {
  ...
  // Get bilingual story data that automatically updates with language changes
  const { data, loading } = useBilingualStoryDetail(story?.id);
  
  // Text-to-speech functionality
  const { speak, speaking, stop } = useTextToSpeech();
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.categoryContainer}>
          <View style={styles.categoryBadge}>
            <Text style={styles.categoryIcon}>{data.categoryIcon}</Text>
